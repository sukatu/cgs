...
<head>
    ...
    <title>Pictures | Corporate Governance Series</title>
...
<body>
    ...
    <main>
        <section class="page-header">
            <div class="container">
                <h1>Pictures</h1>
                <p class="page-subtitle">Event albums with high-resolution images from CGS sessions and stakeholder convenings.</p>
            </div>
        </section>

        <section class="section content-section" style="padding: 60px 20px; background-color: var(--white);">
            <div class="container" style="max-width: 1400px; margin: 0 auto; padding: 0 40px;">
                <div class="updates-grid" style="margin-bottom: 4rem;">
                    <div class="update-card">
                        <img src="images/gallery/New/Fredan-76.JPG" alt="CGS Event Photo 1" class="gallery-thumbnail">
                        <div class="update-content">
                            <span class="update-category">Event Album</span>
                            <h3>CGS Event Gallery</h3>
                            <p>Highlights from our flagship sessions, featuring panel discussions, networking moments, and keynote presentations.</p>
                            <div style="margin-top: 1.5rem; padding-top: 1rem; border-top: 1px solid var(--divider-grey); font-size: 0.9rem; color: var(--text-charcoal);">
                                <strong>Event:</strong> CGS Events | <strong>Photos:</strong> 110 images
                            </div>
                            <a href="#gallery" class="update-link" style="margin-top: 1rem; display: inline-block;">View Full Gallery →</a>
                        </div>
                    </div>

                    <div class="update-card">
                        <img src="images/gallery/New/Fredan-62.JPG" alt="CGS Event Photo 2" class="gallery-thumbnail">
                        <div class="update-content">
                            <span class="update-category">Event Album</span>
                            <h3>Panel Discussions & Sessions</h3>
                            <p>Snapshots and highlights from our panel discussions, featuring governance experts and industry leaders.</p>
                            <div style="margin-top: 1.5rem; padding-top: 1rem; border-top: 1px solid var(--divider-grey); font-size: 0.9rem; color: var(--text-charcoal);">
                                <strong>Series:</strong> CGS Sessions | <strong>Photos:</strong> 120 images
                            </div>
                            <a href="#gallery" class="update-link" style="margin-top: 1rem; display: inline-block;">View Full Gallery →</a>
                        </div>
                    </div>
                    ...
                    <div class="update-card">
                        <img src="images/gallery/New/Fredan-110.JPG" alt="CGS Event Photo 3" class="gallery-thumbnail">
                        <div class="update-content">
                            <span class="update-category">Networking</span>
                            <h3>Stakeholder Networking Sessions</h3>
                            <p>Moments from networking sessions, fostering connections among Africa's governance community.</p>
                            <div style="margin-top: 1.5rem; padding-top: 1rem; border-top: 1px solid var(--divider-grey); font-size: 0.9rem; color: var(--text-charcoal);">
                                <strong>Event:</strong> Various | <strong>Photos:</strong> 120 images
                            </div>
                            <a href="#gallery" class="update-link" style="margin-top: 1rem; display: inline-block;">View Full Gallery →</a>
                        </div>
                    </div>
                ...
                <div id="gallery" style="margin-top: 4rem; max-width: 1400px; margin-left: auto; margin-right: auto; padding: 0 40px; box-sizing: border-box; width: 100%;">
                    <h2 class="section-title" style="text-align: center; font-size: 2.5rem; margin-bottom: 1rem; color: var(--primary-navy); font-weight: 700;">Event Gallery</h2>
                    <p class="section-subtitle" style="text-align: center; font-size: 1.2rem; color: var(--text-light); max-width: 600px; margin: 0 auto 3rem; line-height: 1.6;">
                        Browse through our collection of event photos
                    </p>
                    
                    <div class="gallery-grid" id="imageGallery" style="width: 100%; max-width: 100%; margin: 3rem auto 0; padding: 0; box-sizing: border-box;">
                        <!-- Images will be loaded dynamically -->
                    </div>
                </div>

                <div style="background-color: var(--bg-offwhite); padding: 2rem; border-radius: 8px; margin-top: 3rem; text-align: center;">
                    <p style="color: var(--text-charcoal); line-height: 1.7;">
                        <strong>Image Credits:</strong> All photos from CGS events. High-resolution images available upon request for media use. 
                        <a href="contact.php" style="color: var(--accent-gold); font-weight: 600;">Contact us</a> for licensing information.
                    </p>
                </div>
            </div>
        </section>
    ...
    <script>
        // Load gallery images from the New folder and uploaded images
        async function loadGalleryImages() {
            const galleryContainer = document.getElementById('imageGallery');
            if (!galleryContainer) return;

            // Generate image paths for static images (110 images from Fredan-1.JPG to Fredan-110.JPG)
            const staticImages = [];
            for (let i = 1; i <= 110; i++) {
                staticImages.push({
                    path: `images/gallery/New/Fredan-${i}.JPG`,
                    alt: `CGS Event Photo ${i}`,
                    category: null
                });
            }
            ...
            // Combine static and uploaded images (uploaded images first, then static)
            const allImages = [...uploadedImages, ...staticImages];
            ...
            allImages.forEach((imageData, index) => {
                ...
                galleryItem.innerHTML = `
                    <img src="${imageData.path}" alt="${imageData.alt}" loading="lazy">
                `;
                
                // Add click handler for lightbox view
                galleryItem.addEventListener('click', () => {
                    const imagePaths = allImages.map(img => img.path);
                    openImageModal(imageData.path, index, imagePaths);
                });
                
                galleryContainer.appendChild(galleryItem);
            });

            // Update photo counts in the cards
            const totalCount = allImages.length;
            document.querySelectorAll('[style*="Photos:"]').forEach(el => {
                const currentText = el.innerHTML;
                el.innerHTML = currentText.replace(/Photos:.*?\d+/, `Photos: ${totalCount} images`);
            });
        }

        // Open image in modal/lightbox
        function openImageModal(imagePath, currentIndex, allImages) {
            ...
            if (!modal) {
                ...
                document.addEventListener('keydown', function handleKeydown(e) {
                    ...
                    let currentImgIndex = parseInt(modal.dataset.currentIndex || currentIndex);
                    
                    if (e.key === 'ArrowLeft') {
                        currentImgIndex = (currentImgIndex - 1 + allImages.length) % allImages.length;
                        modal.dataset.currentIndex = currentImgIndex;
                        modal.querySelector('#modalImage').src = allImages[currentImgIndex];
                        modal.querySelector('#imageCounter').textContent = `${currentImgIndex + 1} / ${allImages.length}`;
            ...
            prevBtn.onclick = (e) => {
                ...
                currentImgIndex = (currentImgIndex - 1 + allImages.length) % allImages.length;
                modal.dataset.currentIndex = currentImgIndex;
                modalImage.src = allImages[currentImgIndex];
                imageCounter.textContent = `${currentImgIndex + 1} / ${allImages.length}`;
            };
